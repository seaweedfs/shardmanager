...
func CreateShard(db *sql.DB, shard *Shard) error {
	query := `
		INSERT INTO shards (id, node_id, type, size, status, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
		RETURNING created_at, updated_at
	`
	return db.QueryRow(query, shard.ID.String(), shard.NodeID.String(), shard.Type, shard.Size, shard.Status).Scan(&shard.CreatedAt, &shard.UpdatedAt)
}

// GetShard retrieves a shard by ID
func GetShard(db *sql.DB, id uuid.UUID) (*Shard, error) {
	query := `
		SELECT id, node_id, type, size, status, created_at, updated_at
		FROM shards
		WHERE id = ?
	`
	...
	err := db.QueryRow(query, id.String()).Scan(&sid, &nodeID, &stype, &size, &status, &createdAt, &updatedAt)
	if err != nil {
		return nil, err
	}
...
func ListShards(db *sql.DB) ([]*Shard, error) {
	...
	for rows.Next() {
		...
		var size int64
		var createdAt, updatedAt time.Time
		err := rows.Scan(&sid, &nodeID, &stype, &size, &status, &createdAt, &updatedAt)
		if err != nil {
			return nil, err
		}
		parsedNodeID := uuid.MustParse(nodeID)
...
func UpdateShard(db *sql.DB, shard *Shard) error {
	query := `
		UPDATE shards
		SET node_id = ?, type = ?, size = ?, status = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`
	_, err := db.Exec(query, shard.NodeID.String(), shard.Type, shard.Size, shard.Status, shard.ID.String())
	return err
}
